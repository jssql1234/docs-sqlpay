---
title: Tax Benefit
description: A guide to perform Tax Benefit in SQL Payroll
slug: /miscellaneous/tax-benefit
tags: ["SQL Payroll", "Tax"]
---

## Process Tax Benefit

1. Process Month End

   ![process-month-end](../../static/img/miscellaneous/tax-benefit/process-month-end.png)

2. Select the employee (eg : Lee Chong Wai )

   ![select-employee](../../static/img/miscellaneous/tax-benefit/select-employee.png)

3. Right click on an empty space and click on “Show Tax Benefit”

   ![show-tax-benefits](../../static/img/miscellaneous/tax-benefit/show-tax-benefits.png)

4. Insert the tax benefit and also the amount.

   ![insert-tax-benefits](../../static/img/miscellaneous/tax-benefit/insert-tax-benefits.png)

5. Click code to select the tax benefit.

   ![select-tax-benefits](../../static/img/miscellaneous/tax-benefit/select-tax-benefits.png)

   | Code          | EA Form (Section B)                |
   | ------------- | ---------------------------------- |
   | Motorcar      | 2(a.i)                             |
   | Driver        | 2(a.ii)                            |
   | Furnishing    | 2(c.iii – Perabot dan Kelengkapan) |
   | Entertainment | 2(c.iii – Hiburan dan Rekreasi)    |
   | Gardener      | 2(d)                               |
   | Servant       | 2(d)                               |
   | Vacation      | 2(e)                               |
   | Membership    | 2(c.iii – Hiburan dan Rekreasi)    |
   | Miscellaneous | 2(f)                               |
   | VOLA          | 3                                  |

6. For example, I key in RM100 for “Motorcar”:

   - Select the tax benefit/ key in amount/ Click Save

     ![example-save](../../static/img/miscellaneous/tax-benefit/example-save.png)

7. Now, we go to preview EA Form to check the tax benefit amount:

   - Payroll -> Statutary Reports (Yearly) -> Print Income Tax EA Form...

     ![print-ea](../../static/img/miscellaneous/tax-benefit/print-ea.png)

8. Select the employee and click "**Apply**":

   ![apply-ea](../../static/img/miscellaneous/tax-benefit/apply-ea.png)

9. Click preview and select any format :

   ![preview-ea](../../static/img/miscellaneous/tax-benefit/preview-ea.png)

10. Check the EA Form

    **\*\*Nilai Kereta dan petrol = RM100**

    ![sample-ea](../../static/img/miscellaneous/tax-benefit/sample-ea.png)
